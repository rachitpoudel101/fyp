...
urlpatterns = [
    ...
    path(
        "warehouse-dashboard/", views.warehouse_dashboard, name="warehouse_dashboard"
    ),  # Warehouse dashboard
    path(
    ...
    path(
        "customer/dashboard/", views.customer_dashboard, name="customer_dashboard"
    ),  # Customer dashboard
    path(
        "staff/dashboard/", views.staff_dashboard, name="staff_dashboard"
    ),  # Staff dashboard
    ...
    path(
        "verify-email/<str:token>/", views.verify_email, name="verify_email"
    ),  # Email verification (Customer)
    path(
        "verify-pending/", views.verify_pending, name="verify_pending"
    ),  # Pending verification (Customer)
    path(
        "resend-verification-email/",
        views.resend_verification_email,
        name="resend_verification_email",
    ),  # Resend verification email (Customer)
    path(
        "request-verification/", views.request_verification, name="request_verification"
    ),  # Request verification (Customer)
    # Order creation and details URLs
    path(
        "create_order/", views.create_order, name="create_order"
    ...
    path(
        ...
        inventory_views.update_category,
        name="update_category",
    ),
    path(
        "categories/<int:category_id>/delete/",
        inventory_views.delete_category,
        name="delete_category",
    ),
    path("billing/", views.billing, name="billing"),  # Billing management (Admin)
    # Warehouse management URLs
    path(
        "create_warehouse/", views.create_warehouse, name="create_warehouse"
    ),  # Create warehouse (Admin)
    path(
        "manage_warehouses/", views.manage_warehouses, name="manage_warehouses"
    ),  # Manage warehouses (Admin)
    path(
        "edit_warehouse/<int:warehouse_id>/",
        views.edit_warehouse,
        name="edit_warehouse",
    ),  # Add this line
    path(
        "delete-warehouse/<int:warehouse_id>/",
        views.delete_warehouse,
        name="delete_warehouse",
    ),  # Add this line
    path(
        "add-warehouse-manager/",
        views.add_warehouse_manager,
        name="add_warehouse_manager",
    ),  # Add warehouse manager URL
    path("add-staff/", views.add_staff, name="add_staff"),  # Add staff URL
    path(
        "assign-warehouse-manager/<int:warehouse_id>/",
        views.assign_warehouse_manager,
        name="assign_warehouse_manager",
    ),
    path(
        "warehouse/<int:warehouse_id>/products/",
    ...
    path(
        "admin-management/", views.admin_management, name="admin_management"
    ),  # Admin management page
    path(
        "edit-admin/<int:admin_id>/", views.edit_admin, name="edit_admin"
    ),  # Edit admin
    path(
        "delete-admin/<int:admin_id>/", views.delete_admin, name="delete_admin"
    ),  # Delete admin
    path(
        "change-admin-password/<int:admin_id>/",
    ...
    path(
        ...
        views.get_category_details,
        name="get_category_details",
    ),
    # Add the new API endpoint for warehouse product count
    path(
        "api/warehouse/<int:warehouse_id>/product-count/",
        views.warehouse_product_count,
        name="warehouse_product_count",
    ),
    ...
    path(
        "verify-khalti/",
        csrf_exempt(views.verify_khalti_payment),
        name="verify_khalti_payment",
    ),
    # Fix: Use update_order_status instead of update_delivery_status (doesn't exist)
    path(
        "update-delivery-status/<int:order_id>/",
        views.update_order_status,
        name="update_delivery_status_alt",
    ),
    ...
    path(
        "get-category-details/<int:category_id>/",
        views.get_category_details,
        name="get_category_details",
    ),
    # Notification URL
    ...
    path(
        "export-billing/<int:order_id>/",
        views.export_billing,
        name="export_billing_order",
    ),
    # Email bill route
    ...
    path(
        "assign-staff-to-order/<int:order_id>/",
        views.assign_staff_to_order,
        name="assign_staff_to_order",
    ),
    # Add the new URL pattern for assigning order to warehouse
    path(
        "assign-order-to-warehouse/<int:order_id>/",
        views.assign_order_to_warehouse,
        name="assign_order_to_warehouse",
    ),
    # Staff management URLs
    path(
        "staff/order-management/", views.staff_order_management, name="order_management"
    ),
    path(
        "staff/inventory-management/",
        views.staff_inventory_management,
        name="inventory_management",
    ),
    path(
        "staff/customer-management/",
        views.staff_customer_management,
        name="customer_management",
    ),
    path("staff/deliveries/", views.staff_deliveries, name="deliveries"),
    path(
        "staff/process-order/<int:order_id>/",
        views.staff_process_order,
        name="process_order",
    ),
    # Staff account management URLs
    path("staff/account/", views.staff_account, name="staff_account"),
    path(
        "staff/account/update/", views.staff_update_profile, name="staff_update_profile"
    ),
    path(
